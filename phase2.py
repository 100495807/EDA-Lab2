    ...
    def _find_dist_k(self, node, k, dist, lista, visited):
        ...
        if node not in visited:
            ...
            if dist == k:
                ...
                self._find_dist_k(node.left, k, dist + 1, lista, visited)
                self._find_dist_k(node.right, k, dist + 1, lista, visited)
                ...
                if (parent and (parent.elem, parent.left, parent.right) != (
                    ...
                    self._find_dist_k(parent, k, dist + 1, lista, visited)
...
def _insert1(node, tree):
    ...
    tree.insert(node.elem)  # función insert que viene en el programa
...
def _insert2(node1, node2, tree):
    ...
    if node1.elem == node2.elem:
        tree.insert(node1.elem)  # función insert que viene en el programa
...
def _insert3(tree: BinarySearchTree, node2: BinaryNode,
    ...
    node = tree.search(node2.elem)  # función search que viene en el programa

    if not node:
        new_tree.insert(node2.elem)  # función insert que viene en el programa
